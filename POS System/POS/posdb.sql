...
CREATE SCHEMA IF NOT EXISTS `posdb` DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci ;
USE `posdb` ;
...
CREATE  TABLE IF NOT EXISTS `posdb`.`product` (
  ...
  `buttontextfontsize` INT NULL ,
  `buttontextbold` VARCHAR(10) NULL ,
  `image` LONGBLOB NULL ,
...
-- -----------------------------------------------------
-- Table `posdb`.`depertment`
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `posdb`.`depertment` (
  `id` INT NOT NULL AUTO_INCREMENT ,
  `name` VARCHAR(100) NULL ,
  PRIMARY KEY (`id`) ,
  UNIQUE INDEX `name_UNIQUE` (`name` ASC) )
